...
void test_command_with_timing(int fd, const char* command, const char* name, int delay_ms) {
    printf("\n=== Testing %s command: '%s' ===\n", name, command);
    
    // Flush buffer
    tcflush(fd, TCIOFLUSH);
    
    // Wait before sending (interWriteDelay)
    usleep(10000); // 10ms
    ...
    // Use select with timeout (blockReadTimeout = 2000ms)
    fd_set read_fds;
...
void test_tunerstudio_data_command(int fd) {
    ...
    // For basic test, let's try: "r0000000000" (r + CAN_ID=0 + offset=0 + count=0)
    ...
    for (int i = 0; i < 4; i++) {
        test_command_with_timing(fd, data_commands[i], "Data", 100); // 100ms delay
...
int main() {
    ...
    printf("Proper TunerStudio Protocol Speeduino Test on %s\n", port);
    printf("Based on INI file analysis with correct timing\n\n");
    ...
    printf("Waiting 1 second after port open (delayAfterPortOpen=1000)...\n");
    
    // Wait after port open (delayAfterPortOpen=1000)
    usleep(1000000); // 1 second
    
    printf("Starting communication tests...\n\n");
    
    // Test basic TunerStudio commands with proper timing
    test_command_with_timing(fd, "Q", "Query", 100);
    usleep(1000000);
    
    test_command_with_timing(fd, "S", "Version Info", 100);
    usleep(1000000);
    
    test_command_with_timing(fd, "V", "Version", 100);
    usleep(1000000);
    
    test_command_with_timing(fd, "A", "Data (legacy)", 100);
    usleep(1000000);
